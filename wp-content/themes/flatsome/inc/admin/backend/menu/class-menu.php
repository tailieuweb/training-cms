...
class Menu {
	 ...
	 */
	public function add_menu_fields( $item_id, $item, $depth, $args, $id ) {
	 ...
	 */
	public function update_menu_fields( $menu_id, $menu_item_db_id, $args ) {
		foreach ( $this->fields as $field ) {
			$key = 'menu-item-' . $field;

			if ( isset( $_POST[ $key ] ) && ! empty( $_POST[ $key ] ) && is_array( $_POST[ $key ] ) && isset( $_POST[ $key ][ $menu_item_db_id ] ) ) {
	...
	public function register_assets( $hook ) {
		if ( 'nav-menus.php' === $hook ) {
			...
			wp_enqueue_script( 'flatsome-admin-menu', get_template_directory_uri() . '/assets/js/admin-menu.js', array( 'jquery' ), $version, true );
			wp_enqueue_style( 'flatsome-admin-menu', get_template_directory_uri() . '/assets/css/admin/admin-menu.css', null, $version, 'all' );
		}
	}
}
