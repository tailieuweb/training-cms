				...
				?>
					<li class="post-date meta-wrapper">
						<span class="meta-text">
							...
							if (!is_single()) {
								$time = get_post();
								$timeup = $time->post_date;
								$newDay = date("d", strtotime($timeup));
								$newMonth = date("m", strtotime($timeup));
								echo '<div class="day">' . $newDay . '</div>';
								echo '<div class="month">Thang ' . $newMonth . '</div>';
							} else { ?>
