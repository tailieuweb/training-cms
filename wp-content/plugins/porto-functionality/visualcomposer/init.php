	...
	class Porto_VC_Init extends VisualComposer\Framework\Container {
				...
				if ( ! empty( $_REQUEST['post'] ) && 'porto_builder' == get_post_type( $_REQUEST['post'] ) && 'popup' == get_post_meta( $_REQUEST['post'], PortoBuilders::BUILDER_TAXONOMY_SLUG, true ) ) {
					add_action(
						...
						function( $api ) {
							$base_url      = rtrim( plugins_url( basename( dirname( PORTO_FUNC_FILE ) ) ), '\\/' ) . '/visualcomposer';
							$elements_api  = $api->elements;
							$manifest_path = __DIR__ . '/elements/portoPopupContainer/manifest.json';
							$element_url   = $base_url . '/elements/portoPopupContainer';
							$elements_api->add( $manifest_path, $element_url );
						},
						12
					);
		...
		public function load_element_editor_js() {
			if ( current_user_can( 'edit_posts' ) && isset( $_REQUEST['vcv-editable'] ) && ! empty( $_REQUEST['vcv-source-id'] ) ) {
				wp_enqueue_script( 'porto-vc-editor', PORTO_FUNC_URL . 'visualcomposer/assets/live_update.js', array(), PORTO_SHORTCODES_VERSION, true );
			}
		}
		...
		public function load_elements_js() {
			if ( isset( $_REQUEST['vcv-action'] ) && 'frontend' == $_REQUEST['vcv-action'] ) {
				...
				wp_enqueue_script( 'porto-vc-editor', PORTO_FUNC_URL . 'visualcomposer/settings/public/dist/element.bundle.js', array( 'vcv:editors:frontend:script' ), PORTO_SHORTCODES_VERSION, true );
