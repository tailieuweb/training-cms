...
class PortoBlogController extends Container implements Module {

	use EventsFilters;
	use WpFiltersActions;
	public function __construct() {
		if ( ! defined( 'VCV_PORTO_BLOG_CONTROLLER' ) ) {
			$this->addFilter(
				'vcv:editor:variables vcv:editor:variables/portoBlog',
				'getImageSize'
			);
			define( 'VCV_PORTO_BLOG_CONTROLLER', true );
		}
		$this->addFilter( 'vcv:autocomplete:portoBlogIDs:render', 'getBlogIDs' );
	...
	protected function getBlogIDs( $response, $payload ) {
		...
		$product_infos = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT ID AS id, post_title AS title
					FROM {$wpdb->posts} 
					WHERE post_type = 'post' AND post_status = 'publish' AND ( ID = %d OR post_title LIKE '%%%s%%' )",
