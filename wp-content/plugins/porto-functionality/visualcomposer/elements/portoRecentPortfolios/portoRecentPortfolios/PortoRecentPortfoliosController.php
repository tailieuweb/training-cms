<?php
namespace porto\portoRecentPortfolios;
if ( ! defined( 'ABSPATH' ) ) {
	exit;
}
...
class PortoRecentPortfoliosController extends Container implements Module {

	use EventsFilters;
	use WpFiltersActions;
	public function __construct() {
		if ( ! defined( 'VCV_PORTO_RECENT_PORTFOLIOS_CONTROLLER' ) ) {
			$this->addFilter(
				'vcv:editor:variables vcv:editor:variables/portoRecentPortfolios',
				'getPortfolioValues'
			);
			define( 'VCV_PORTO_RECENT_PORTFOLIOS_CONTROLLER', true );
		}
		$this->addFilter( 'vcv:autocomplete:portoRecentPortfolioIDs:render', 'getPortfolioIDs' );
		$this->addFilter( 'vcv:autocomplete:portoRecentPortfolioCatIDs:render', 'getPortfolioCatIDs' );
